...
После установки дайте агенту конкретную задачу по REES46 SDK. Хорошие запросы явно упоминают REES46, JS SDK v3, `r46` или область интеграции, с которой нужно работать.

### Добавить REES46 SDK в проект

```text
Добавь REES46 JS SDK v3 в этот storefront. Инициализируй SDK один раз, сделай код безопасным для SSR и покажи, откуда должен приходить SHOP_TOKEN.
```

### Проверить существующую интеграцию

```text
Проверь текущую интеграцию REES46 JS SDK в проекте. Найди дублирующиеся init-вызовы, проблемы с SPA-роутингом, cart и purchase tracking, а также недокументированные r46-команды.
```

### Отладить трекинг

```text
Найди, почему события REES46 product view и cart отправляются некорректно. Проверь текущие r46-вызовы и дай чеклист проверки в браузере.
```

### Добавить рекомендации

```text
Добавь рекомендации REES46 на страницу товара. Используй product-related рекомендации, сохрани recommendation attribution для add-to-cart событий и не используй недокументированные SDK-вызовы.
```

### Проверить границы безопасности

```text
Проверь эту интеграцию REES46 на browser-side secrets и server-only API usage. Все небезопасное вынеси в backend-рекомендации, а не в клиентский код.
```

### Использовать скилл как project rules

Если агент не загружает `SKILL.md` автоматически, вставьте короткую инструкцию в чат или project rules:

```text
Используй REES46 JS SDK skill из /absolute/path/to/skills/js-sdk.
Перед изменением кода прочитай SKILL.md и релевантные файлы из references/.
Используй только документированные r46-команды и добавляй шаги ручной проверки.
```
